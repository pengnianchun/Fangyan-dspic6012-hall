...
void CAN1_OperationMode(unsigned char mode) {//??æ­¢æ¨¡å¼?
   /*REQOP<2:0>£º ÇëÇó¹¤×÷Ä£Ê½Î»
111 = ÉèÖÃ¼àÌýËùÓÐ±¨ÎÄÄ£Ê½  7
110 = ±£Áô                 6
101 = ±£Áô                 5 
100 = ÉèÖÃÅäÖÃÄ£Ê½         4  
011 = ÉèÖÃ¼àÌýÄ£Ê½         3
...
void CAN1_Init(void) {
    ...
    C1CTRL = CAN_IDLE_CON & CAN_CAPTURE_DIS & CAN_MASTERCLOCK_1 & CAN_REQ_OPERMODE_CONFIG;
    while (C1CTRLbits.OPMODE != 4);

    //??ç½?æ³¢ç?¹ç??1?7
    C1CFG1 = CAN_SYNC_JUMP_WIDTH1 & CAN_BAUD_PRE_SCALE(2);
    ...
    C1RXF0SID = CAN_FILTER_EID(0) & CAN_RX_EID_EN; //?¥æ?¶æ»¤æ³¢å??1?7
    *((unsigned long *) &C1RXF0EIDH) = CAN_FILTER_EID(0);
    C1RXM0SID = CAN_MASK_EID(0) & CAN_MATCH_FILTER_TYPE; //å±??½æ»¤æ³¢å??1?7
    *((unsigned long *) &C1RXM0EIDH) = CAN_MASK_EID(0);
    ...
    //ä¸???ä½¿è??
    C1INTE = 0x0041; //ä½¿è??RX0B?¥æ?¶å???¤é??ä¸???1?7
    _C1IE = 1;

    C1CTRL = CAN_IDLE_CON & CAN_CAPTURE_DIS & CAN_MASTERCLOCK_1 & CAN_REQ_OPERMODE_NOR;
...
void CAN2_Init(void) {
    C2CTRL = CAN_IDLE_CON & CAN_MASTERCLOCK_1 & CAN_REQ_OPERMODE_CONFIG
            & CAN_CAPTURE_DIS;
    while (C2CTRLbits.OPMODE != 4);

    //??ç½?æ³¢ç?¹ç??1?7
    C2CFG1 = CAN_SYNC_JUMP_WIDTH1 & CAN_BAUD_PRE_SCALE(2);
    C2CFG2 = CAN_WAKEUP_BY_FILTER_DIS & CAN_PHASE_SEG2_TQ(7) &
            CAN_PHASE_SEG1_TQ(6) & CAN_PROPAGATIONTIME_SEG_TQ(2) &
            CAN_SEG2_FREE_PROG & CAN_SAMPLE1TIME;

    C2RXF0SID = CAN_FILTER_EID(0) & CAN_RX_EID_EN; //?¥æ?¶æ»¤æ³¢å??1?7
    *((unsigned long *) &C2RXF0EIDH) = CAN_FILTER_EID(0);
    C2RXM0SID = CAN_MASK_EID(0) & CAN_MATCH_FILTER_TYPE; //å±??½æ»¤æ³¢å??1?7
    *((unsigned long *) &C2RXM0EIDH) = CAN_MASK_EID(0);
    //?¥æ?¶æ¨¡å¼?
    C2RX0CON = CAN_RXFUL_CLEAR & CAN_BUF0_DBLBUFFER_DIS;

    //ä¸???ä½¿è??
    C2INTE = 0x0001; //ä½¿è??RX0B?¥æ?¶å???¤é??ä¸???1?7
    _C2IE = 1;

    C2CTRL = CAN_IDLE_CON & CAN_CAPTURE_DIS & CAN_MASTERCLOCK_1 & CAN_REQ_OPERMODE_NOR;
...
void CAN1_ReceiveMessage(CAN_MESSAGE *message) {
    ...
    if (C1RX0SIDbits.RXIDE) {
        id += C1RX0SIDbits.SID; //??EID 28--18
        id <<= 12;
        id += C1RX0EID; //??EID17--6
        id <<= 6;
        id += C1RX0DLCbits.EID5_0; //??EID5--0
        message->type = 0;
    } else { //?¥æ?¶ç???°æ????????å¸?1?7
        id += C1RX0SIDbits.SID; //??SID 10--0
        message->type = 1;
    }
...
void CAN2_ReceiveMessage(CAN_MESSAGE *message) {
    ...
    if (C2RX0SIDbits.RXIDE) {
        id += C2RX0SIDbits.SID; //??EID 28--18
        id <<= 12;
        id += C2RX0EID; //??EID17--6
        id <<= 6;
        id += C2RX0DLCbits.EID5_0; //??EID5--0
        message->type = 0;
    } else { //?¥æ?¶ç???°æ????????å¸?1?7
        id += C2RX0SIDbits.SID; //??SID 10--0
        message->type = 1;
    }
